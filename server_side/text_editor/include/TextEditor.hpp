...
class WorkWithLines : public IWorkWithText {
   ...
   private:
    ...
    AnswerForInsertAction whatPosition(Element* beforeInsert, Element* insertElement, Element* afterElement,
                                       StartOfLine* line);
    AnswerForInsertAction whatPositionDelete(Element* elementToDelete);

    AnswerForInsertAction insertElement(Element* insertElement, Element* afterElement = nullptr,
                                        Element* beforeElement = nullptr);
    AnswerForInsertAction insertElement(Element* insertElement, size_t lineWhereInsert,
    ...
    std::string createCommand(bool isDelete, Element* elementOperation, Element* beforeElement = nullptr,
                              Element* afterElement = nullptr);
